...
`generate-posts` 단계에서 레이아웃 붕괴 가능성이 높은 콘텐츠에 대해 경고를 출력합니다.

- 긴 단일 행: 권장 180자 이하
- 대형 코드블록: 권장 120줄 이하
- 대형 표: 권장 7열 이하

이 경고는 빌드를 중단하지 않으며, 상세 페이지에서 표/코드가 깨질 가능성이 있는 게시글을 사전에 발견하기 위한 용도입니다.
...
- **주의**: `seriesOrder`와 함께 사용해야 사이드바에 표시됩니다. 2개 이상의 게시글이 같은 시리즈를 가져야 표시됩니다.
 - **추가 동작**: 하나의 카테고리에는 여러 시리즈가 공존할 수 있으며, 목록 페이지에서 드롭다운으로 시리즈를 선택할 수 있습니다.
...
- 카테고리 페이지는 `?series=<seriesName>` 쿼리로 현재 선택 시리즈를 유지합니다.
- 잘못된 `series` 값이 전달되면 표시 가능한 시리즈 중 기본값으로 자동 보정됩니다.
- `all` 카테고리에서는 시리즈 사이드바가 노출되지 않으며 `series` 쿼리는 무시됩니다.
